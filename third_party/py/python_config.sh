...
if [ -d "../com_github_argcv_argcv" ]; then
  script_path="../com_github_argcv_argcv"
else
...
[ -f "$script_path/tools/bash_helpers.sh" ] && . "$script_path/tools/bash_helpers.sh"

EXPECTED_PATHS="$script_path/third_party/py/python_include"\
" $script_path/third_party/py/python_lib"\
...
# to create junctions on Windows.
  if is_windows; then
    ...
    ln -sf "${python_include}" third_party/py/python_include
    ln -sf "${python_lib}" third_party/py/python_lib
...
function check_python {
  for x in $EXPECTED_PATHS; do
    if [ ! -e "$x" ]; then
      echo -e "\n\nERROR: Cannot find '${x}'.  Did you run setup?\n\n" 1>&2
      exit 1
    fi
    ...
    if [ ! -d "${true_path}" ]; then
      echo -e "\n\nERROR: '${x}' does not refer to an existing directory: ${true_path}.  Do you need to rerun setup?\n\n" 1>&2
      exit 1
    fi
  done
}
