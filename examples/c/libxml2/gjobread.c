 ...
 *
 *
 * Revised by yuikns@users.noreply.github.com
 *
 * bazel run //examples/c/libxml2:gjobread
 * $(pwd)/examples/c/libxml2/gjobs.xml
 */
...
static gJobPtr parseGjobFile(char *filename ATTRIBUTE_UNUSED) {
  xmlDocPtr doc;
  gJobPtr ret;
  jobPtr curjob;
  xmlNsPtr ns;
  ...
  while (cur != NULL) {
    if ((!xmlStrcmp(cur->name, (const xmlChar *)"Job")) && (cur->ns == ns)) {
      ...
      if (curjob != NULL) ret->jobs[ret->nbJobs++] = curjob;
